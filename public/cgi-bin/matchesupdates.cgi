...
db = MySQLdb.connect(host="localhost", # your host, usually localhost
                     user="newpep_news", # your username
                      passwd="news1234", # your password
                      db="newpep_newspepadb") # name of the data base
...
pivotid = row[0]
   cur.execute("SELECT id,title,description,content FROM raw_stories where status LIKE %s or id LIKE %s", ("%" + statusid + "%","%" + pivotid + "%",))
   for row in cur.fetchall() :
      words = nltk.wordpunct_tokenize(nltk.clean_html(row[3]))
...
for pivot in dic:
   for matches in dic[pivot]:
      ...
      print pivot ,matches 
       ...
       cur.execute('''INSERT into clusters (cluster_pivot,matches,created_date,modified_date)
                   values (%s,%s,%s,%s,%s)''',
                   (pivot,matches,now,now))
       cur.execute ("""
   UPDATE stories
   SET have_cluster=%s, status=%s 
""", (havecluster, status))
